...
class AuthorizeLetter extends StatefulWidget {
  const AuthorizeLetter({super.key});

  @override
  State<AuthorizeLetter> createState() => _AuthorizeLetterState();
}

class _AuthorizeLetterState extends State<AuthorizeLetter> {
  ...
  File? passportDriverImage;
  File? idCardOwnerImage;
        ...
        children: [
          ...
          Expanded(
            ...
            _uploadCard(
              ...
              subtitle: 'Grant/VOC',
              imageFile: vehicleRegistrationImage,
              emptyBadgeText: 'REQUIRED',
              emptyBadgeColor: const Color(0xFFFCE8EB),
              emptyBadgeTextColor: const Color(0xFFE53935),
              onTap: () async {
                final file = await _pickImageSource();
                if (file == null) return;
                setState(() => vehicleRegistrationImage = file);
              },
            ),
            const SizedBox(height: 10),
            _uploadCard(
              title: 'Passport Driver',
              imageFile: passportDriverImage,
              onTap: () async {
                final file = await _pickImageSource();
                if (file == null) return;
                setState(() => passportDriverImage = file);
              },
            ),
            const SizedBox(height: 10),
            _uploadCard(
              title: 'Identification Card Owner',
              imageFile: idCardOwnerImage,
              onTap: () async {
                final file = await _pickImageSource();
                if (file == null) return;
                setState(() => idCardOwnerImage = file);
              },
            ),
            ...
            child: ElevatedButton(
              onPressed: () {
                ...
                if (vehicleRegistrationImage == null ||
                    passportDriverImage == null ||
                    idCardOwnerImage == null) {
                ...
                final double totalPrice = quantity * 5.0;
                Navigator.push(
                  context,
                  MaterialPageRoute(
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          SizedBox(
            width: 120,
            child: Image.asset('assets/authorizeLetter.png', fit: BoxFit.contain),
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'Required when the vehicle owner is not travelling with the car during the trip.',
                  style: TextStyle(
                    fontSize: 12,
  ...
  Widget _uploadCard({
    ...
    String? emptyBadgeText,
    Color emptyBadgeColor = const Color(0xFFEFF4FF),
    Color emptyBadgeTextColor = const Color(0xFF2B6FD6),
  }) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: uploaded ? const Color(0xFF5AC8BE) : const Color(0xFFE37575),
          width: 1.2,
        ),
      ),
      child: Column(
          ...
          Row(
            children: [
              ...
              if (uploaded)
                _badge(
                  text: 'UPLOADED',
                  backgroundColor: const Color(0xFFD9F5EE),
                  textColor: const Color(0xFF26B39E),
                )
              else if (emptyBadgeText != null)
                _badge(
                  text: emptyBadgeText,
                  backgroundColor: emptyBadgeColor,
                  textColor: emptyBadgeTextColor,
                ),
            ],
          ),
          const SizedBox(height: 10),
